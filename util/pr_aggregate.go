...
// pr_report (with pipeline/deployment columns) and pipeline_report.
// Run this after ScrapeRaw to refresh both aggregated views.
func Aggregate(ctx context.Context, db *sql.DB, repos []string, logf LogFunc) error {
	for _, repo := range repos {
		logf("[%s] aggregating pr_report", repo)
		data, depByPipeline, prLinks, err := loadReportData(ctx, db, repo)
		if err != nil {
			return fmt.Errorf("load data for %s: %w", repo, err)
		}
		if err := PopulateReportTable(ctx, db, repo, data, depByPipeline, prLinks); err != nil {
			return fmt.Errorf("populate report for %s: %w", repo, err)
		}
		logf("[%s] aggregated %d PRs", repo, len(data))
	}
	// Also rebuild pipeline_report so the deployment-frequency chart stays current.
	return AggregatePipelines(ctx, db, repos, logf)
}

// loadReportData reads pull_requests, pr_activity, pr_diffstat, pr_pipelines,
// and deployments from the DB and assembles the structures that PopulateReportTable expects.
func loadReportData(ctx context.Context, db *sql.DB, repo string) (
	map[int]*PullRequestReportData,
	map[string]DeploymentData,
	map[int][]string,
	error,
) {
	...
	prLinks, err := QueryPRPipelineLinks(ctx, db, repo)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	depByPipeline := make(map[string]DeploymentData, len(deployments))
	for _, d := range deployments {
		depByPipeline[d.Pipeline.UUID] = d
	}
	return mapPrWithOtherData(prs, activities, diffstats), depByPipeline, prLinks, nil
}
